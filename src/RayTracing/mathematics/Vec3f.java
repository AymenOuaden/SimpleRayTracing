...
/**
 * Basic class to represent 3-vectors Not intended to be a complete algebra
 * class ! Methods are studied to avoid allocation of new objects, this explains
 * some weird choices...
 *
...
public class Vec3f {
	...
	/**
	 * x, y and z values of the current vector. These are public to allow fast
	 * access and simple use.
	 */
	public float x, y, z;
	...
	public Vec3f(final float x, final float y, final float z, final double w) {
		this.x = x;
		this.y = y;
		this.z = z;
		this.w = w;
	...
	/**
	 * Fill current vector with the cross product of two vectors. Take care of
	 * parameters order, cross-product is anti-commutative!
	 *
	...
	public float distance(Vec3f b) {
		return (float) Math.sqrt(Math.pow(this.x - b.x, 2) + Math.pow(this.y - b.y, 2) + Math.pow(this.z - b.z, 2));
	}
